...
for i in range(0, len(r)):
   r[i] = r[i]-r0
figure(1)
semilogx(r,fp_real,'g',label='Real.Phi_{+}')
figure(2)
semilogx(r,fp_imag,'g',label='Imag.Phi_{+}')

fn_real = zeros(len(fpr))
for i in range(0, len(fpr)):
   fn_real[i] = fnr[i].real

fn_imag = zeros(len(fpr))
for i in range(0, len(fpr)):
   fn_imag[i] = fnr[i].imag

figure(1)
semilogx(r,fn_real,'b',label='Real.Phi_{-}')
figure(2)
semilogx(r,fn_imag,'b',label='Imag.Phi_{-}')
